 ...
 *  인스턴스 내부클래스:
 *  static 내부클래스:
 *  지역내부클래스 : 매서드 내에 선언된 클래스. 매서드 내부에서만 가용이 가능함.
 *  무명 클래스 :
 */
...
class Outer1 {
	...
	void method() {
		class LocalInner { //지역 내부 클래스
			int iv = 600; static int cv = 700; final static int MAX = 700;	}
		LocalInner lc = new LocalInner();
		System.out.println(lc.iv);///600
		System.out.println(lc.MAX);///700
		System.out.println(LocalInner.MAX);
		System.out.println(LocalInner.cv);//700
	}
	void method2() {
		//LocalInner lc = new LocalInner(); //=> method() 메서드 안에서만 사용가능
	}
	}
